...
describe('authoritative special-action guards', () => {
  ...
  it('does not allow GamePlay to write the authoritative special-action fields directly', () => {
    expect(gamePlay).not.toMatch(/updateDoc\(doc\(db,\s*['"]games['"][\s\S]{0,700}cazadorPendingShot/);
    expect(gamePlay).not.toMatch(/updateDoc\(doc\(db,\s*['"]games['"][\s\S]{0,700}chivoPendingChoice/);
    expect(gamePlay).not.toMatch(/updateDoc\(doc\(db,\s*['"]games['"][\s\S]{0,700}voteBanned/);
  });
});
