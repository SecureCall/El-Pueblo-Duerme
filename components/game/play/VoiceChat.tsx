...
// The game state is the source of truth for team membership. These three
// roles are kept as a defensive fallback for older game documents that may
// not have wolfTeam populated.
const LEGACY_WOLF_ROLES = new Set(['Lobo', 'Lobo Blanco', 'Cría de Lobo']);

export function VoiceChat({ gameId, userId, userName, phase, myRole, isAlive, wolfTeam = {} }: Props) {
  const [collapsed, setCollapsed] = useState(false);

  const isWolf = wolfTeam[userId] === true || LEGACY_WOLF_ROLES.has(myRole);
  ...
  return (
    <div className={`fixed bottom-20 left-3 z-50 w-52 rounded-2xl border ${cfg.border} ${cfg.bg} backdrop-blur-md shadow-2xl`}>
      <button onClick={() => setCollapsed(c => !c)} className="w-full flex items-center justify-between px-3 py-2.5 text-white hover:bg-white/5 rounded-t-2xl transition-colors">
        ...
        {collapsed ? <ChevronUp className="h-3 w-3 text-white/40" /> : <ChevronDown className="h-3 w-3 text-white/40" />}
      </button>

      {!collapsed && (
        <div className="px-3 pb-3 space-y-2 border-t border-white/10">
          {error && <p className="text-red-400 text-[10px] pt-2 text-center leading-snug">{error}</p>}

          {!permissionGranted && !error && (
            <button onClick={joinVoice} disabled={joining} className={`w-full mt-2 flex items-center justify-center gap-2 py-2 rounded-xl ${cfg.joinBtn} text-white text-[11px] font-bold transition-all shadow ${joining ? 'opacity-60 cursor-wait' : 'hover:scale-[1.02]'}`}>
          ...
          {canSpeak && permissionGranted && (
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-1.5">
                <div className={`w-5 h-5 rounded-full bg-white/10 flex items-center justify-center text-[10px] ${!isMuted ? 'ring-1 ring-green-400' : ''}`}>🧑</div>
                <span className="text-[11px] text-white/80 truncate max-w-[70px]">Tú</span>
              </div>
              <button onClick={toggleMute} className={`w-7 h-7 rounded-full flex items-center justify-center transition-all shadow ${isMuted ? 'bg-red-700 hover:bg-red-600' : 'bg-green-700 hover:bg-green-600'}`} title={isMuted ? 'Activar micrófono' : 'Silenciar'}>
                {isMuted ? <MicOff className="h-3 w-3 text-white" /> : <Mic className="h-3 w-3 text-white" />}
              </button>
            </div>
          )}

          {!canSpeak && permissionGranted && <p className="text-white/40 text-[10px] pt-2 flex items-center gap-1"><span>👂</span> Solo escuchas</p>}
          ...
          {peers.length === 0 && permissionGranted && <p className="text-white/25 text-[10px] pt-1">Nadie más en el canal</p>}
