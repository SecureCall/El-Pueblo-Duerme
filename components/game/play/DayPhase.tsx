...
import { requestBansheePrediction } from '@/lib/game/bansheePrediction';
...
export function DayPhase({ game, gameId, myRole, me, userId, userName, isHost, onVote, onJuezSecondVote, onAlborotadoraFight, votesFromSub = {}, onTimerEnd }: Props) {
  ...
  const submitBansheePrediction = async (targetUid: string) => {
    ...
    try {
      await requestBansheePrediction(gameId, targetUid);
    } catch (e) {
  ...
  const sendMsg = async (e: React.FormEvent) => {
    ...
    setSending(true);
    await addDoc(collection(db, 'games', gameId, 'publicChat'), { senderId: userId, senderName: userName, text: msg.trim(), createdAt: serverTimestamp() });
