...
export async function processNight(db: Firestore, gameId: string) {
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            if (hechicera && wolfTarget && !hechicera.potions?.save) {
                // This is where a human Hechicera would be prompted. For bots, we can add logic here.
                // For now, we assume reactive part is handled by player. If they used save potion it's in allProtectedIds.
            }
