...
import { canWatchVideo } from '@/lib/firebase/coins';
...
export function VideoReward({ onCoinsEarned }: VideoRewardProps) {
  ...
  const rewardUser = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'No se pudo conceder la recompensa');
  ...
  return (
    <div className="bg-gradient-to-br from-yellow-900/40 to-orange-900/40 border border-yellow-500/30 rounded-2xl p-6">
      ...
      {phase === 'idle' && (
        <button onClick={startVideo} className="w-full flex items-center justify-center gap-3 bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-4 rounded-xl transition-all active:scale-95">
      ...
      {phase === 'watching' && (
        <div className="space-y-3">
          ...
          <div className="bg-white/10 rounded-full h-2 overflow-hidden">
            <div className="bg-yellow-400 h-full transition-all duration-1000" style={{ width: `${progress}%` }} />
          </div>
      ...
      {phase === 'done' && (
        <div className="text-center space-y-3">
          ...
          <button onClick={reset} className="bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-2 px-6 rounded-xl transition-all">
            Ver otro vídeo
          </button>
        </div>
      )}
