...
export async function processVotes(transaction: Transaction, gameRef: DocumentReference<Game>) {
    ...
    if (lynchedPlayerId) {
        const result = await killPlayer(transaction, gameRef, game, lynchedPlayerId, 'vote_result', `El pueblo ha hablado. ${game.players.find(p => p.userId === lynchedPlayerId)?.displayName} ha sido linchado.`);
