...
export async function runAIActions(gameId: string, phase: 'day' | 'night') {
    ...
    try {
        ...
        if (phase === 'night') {
            const aiPlayersToDoAction = fullPlayers.filter(p => p.isAI && p.isAlive && !p.usedNightAbility);
            ...
            const aiPlayersToVote = fullPlayers.filter(p => p.isAI && p.isAlive && !p.votedFor);
...
export async function runAIHunterShot(gameId: string) {
    ...
    try {
        ...
        if (game.phase !== 'hunter_shot' || !game.pendingHunterShot) {
            return;
        }
