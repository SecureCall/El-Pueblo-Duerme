...
export async function processNight(db: any, gameId: string) {
...
export async function processVotes(db: any, gameId: string) {
  const gameRef = doc(db, 'games', gameId);
    ...
    await runTransaction(db, async (transaction: Transaction) => {
      ...
      if (mostVotedPlayerIds.length > 1 && !isTiebreaker) {
           if(game.settings.juryVoting) {
                 game.events.push({ id: `evt_vote_tie_${game.currentRound}`, gameId, round: game.currentRound, type: 'vote_result', message: `¡La votación resultó en un empate! El jurado de los muertos decidirá el destino de: ${mostVotedPlayerIds.map(id => game.players.find(p=>p.userId === id)?.displayName).join(', ')}.`, data: { tiedPlayerIds: mostVotedPlayerIds, final: false }, createdAt: Timestamp.now() });
...
export async function processJuryVotes(db: any, gameId: string) {
     const gameRef = doc(db, 'games', gameId);
      try {
        await runTransaction(db, async (transaction: Transaction) => {
