...
export async function checkGameOver(gameData: Game, lynchedPlayer?: Player | null): Promise<{ isGameOver: boolean; message: string; winnerCode?: string; winners: string[] }> {
    ...
    if (lynchedPlayer) {
        ...
        // Special win condition for Executioner
        if (gameData.settings.executioner) {
            ...
            if (executioner && executioner.executionerTargetId === lynchedPlayer.userId) {
                // If the executioner's target was the Drunk Man, both can win.
                const winners = [executioner.userId, ...sharedWinners];
                return {
                    isGameOver: true,
    ...
    // This handles the solo win for Drunk Man if no other condition is met
...
export async function processNight(transaction: Transaction, gameRef: DocumentReference) {
    ...
    actions.forEach((action: NightAction) => {
         ...
         if (action.actionType === 'fairy_find' && targetIndex !== -1 && game.players[targetIndex].role === 'sleeping_fairy') {
             ...
             game.events.push({ id: `evt_fairy_found_${Date.now()}`, gameId: game.id, round: game.currentRound, type: 'special', message: `¡Las hadas se han encontrado! Un nuevo poder ha despertado.`, data: {}, createdAt: Timestamp.now() });
         }
    ...
    if (game.leprosaBlockedRound !== game.currentRound) {
        const wolfVotes = actions.filter((a: NightAction) => a.actionType === 'werewolf_kill').map((a: NightAction) => a.targetId);
        const getConsensusTarget = (votes: string[]) => {
            ...
            return mostVotedTargets.length > 0 ? mostVotedTargets[0] : null; // Pick first on tie
        };
    ...
    actions.filter((a: NightAction) => ['doctor_heal', 'guardian_protect', 'priest_bless', 'hechicera_save'].includes(a.actionType)).forEach((a: NightAction) => allProtectedIds.add(a.targetId));
    ...
    if (game.wolfCubRevengeRound === game.currentRound) {
         game.events.push({ id: `evt_revenge_${Date.now()}`, gameId: game.id, round: game.currentRound, type: 'special', message: "¡La cría de lobo ha muerto! La manada, enfurecida, atacará de nuevo.", data: {}, createdAt: Timestamp.now() });
    ...
    game.events.push({ id: `evt_night_${game.currentRound}`, gameId: game.id, round: game.currentRound, type: 'night_result', message: nightMessage, data: { killedPlayerIds: newlyKilledPlayers.map(p => p.userId), savedPlayerIds: Array.from(allProtectedIds) }, createdAt: Timestamp.now() });
...
export async function processVotes(transaction: Transaction, gameRef: DocumentReference) {
  const gameSnap = await transaction.get(gameRef);
  ...
  const lastVoteEvent = [...game.events].sort((a, b) => toPlainObject(b.createdAt).getTime() - toPlainObject(a.createdAt).getTime()).find(e => e.type === 'vote_result' && e.round === game.currentRound);
  ...
  if (mostVotedPlayerIds.length > 1 && !isTiebreaker) {
       if(game.settings.juryVoting) {
             game.events.push({ id: `evt_vote_tie_${game.currentRound}`, gameId: game.id, round: game.currentRound, type: 'vote_result', message: `¡La votación resultó en un empate! El jurado de los muertos decidirá el destino de: ${mostVotedPlayerIds.map(id => game.players.find(p=>p.userId === id)?.displayName).join(', ')}.`, data: { tiedPlayerIds: mostVotedPlayerIds, final: false }, createdAt: Timestamp.now() });
             game.players.forEach(p => { p.votedFor = null; });
             const phaseEndsAt = Timestamp.fromMillis(Date.now() + PHASE_DURATION_SECONDS * 1000);
             transaction.update(gameRef, toPlainObject({ players: game.players, events: game.events, phase: 'jury_voting', phaseEndsAt }));
       } else {
...
export async function processJuryVotes(transaction: Transaction, gameRef: DocumentReference) {
    ...
    if (lynchedPlayerId) {
         ...
         game.events.push({ id: `evt_jury_tie_${game.currentRound}`, gameId: game.id, round: game.currentRound, type: 'vote_result', message: "El jurado no ha llegado a un acuerdo. Nadie es linchado.", data: { lynchedPlayerId: null, final: true }, createdAt: Timestamp.now() });
    }
