...
export async function startGame(gameId: string, creatorId: string) {
    ...
    try {
        ...
        }, 15000); // 15 second delay to show roles
...
export async function getSeerResultServer(gameId: string, seerId: string, targetId: string) {
    const { firestore } = getSdks();
    try {
...
export async function submitHunterShotServer(gameId: string, hunterId: string, targetId: string) {
    const { firestore } = getSdks();
    const gameRef = doc(firestore, 'games', gameId) as DocumentReference<Game>;
...
export async function submitTroublemakerActionServer(gameId: string, troublemakerId: string, target1Id: string, target2Id: string) {
    const { firestore } = getSdks();
    const gameRef = doc(firestore, 'games', gameId) as DocumentReference<Game>;
