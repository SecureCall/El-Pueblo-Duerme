    ...
    try {
        ...
        if (phase === 'night') {
            ...
            for (const ai of aiPlayersToDoAction) {
                ...
                // We need to call the client-side action from here, which is not directly possible.
                // This architecture requires a rethink. For now, we will assume this could trigger a client-side function if this were a fullstack app with websockets.
                // Since this is server-side, we can't call a client-side action. We will log the intent for now.
                console.log(`AI ${ai.displayName} wants to perform ${actionType} on ${targetId}, but this needs to be triggered client-side.`);
                // To make it work, we'd need to use the server SDK to directly write the night action.
                // For the purpose of this fix, let's just log it.
            }
        } else if (phase === 'day') {
