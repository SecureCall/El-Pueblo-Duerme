...
export function GameRoom({ gameId }: { gameId: string }) {
  ...
  const { game, players, events, messages, wolfMessages, twinMessages, loading, error } = useGameState(gameId);
  ...
  const renderContent = () => {
    ...
    switch (game.status) {
        ...
        case 'in_progress':
        case 'finished':
            return <GameBoard game={game} players={players} currentPlayer={currentPlayer} events={events} messages={messages} wolfMessages={wolfMessages} twinMessages={twinMessages} />;
        default:
